...
class StockConsumption(Factor):
    ...
    def desc(self) -> str:
        return "consumption data factor"
    ...
    def run(self, start_time: datetime, end_time: datetime) -> Tuple[pd.DataFrame, Exception]:
        ...
        origin_dir = '/mnt/Q/users/shijiachen/消费数据/results'
        ...
        # src_df = pd.read_csv('./20230508.csv')
        ...
        for col_name in columns:
            if 'named' in col_name:
                new_column_dict[col_name] = 'datetime'
...
if __name__ == '__main__':
    now = datetime.now()
    obj = StockConsumption()

    try:
        df, err = obj.run(datetime(2016, 5, 1), now)
    ...
    print(df, err)
    df.to_pickle("StockConsumption.pkl")
